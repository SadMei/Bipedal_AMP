...
def track_lin_vel_xy_base_frame_exp(
    ...
    """Reward base-frame linear velocity tracking using an exponential kernel.

    UniformVelocityCommand defines its linear velocity command in the robot base
    frame, so the measured velocity must use the same frame.
    """
    # extract the used quantities (to enable type-hinting)
    asset = env.scene[asset_cfg.name]
...
def track_lin_vel_xy_yaw_frame_exp(
    ...
    """Backward-compatible name for base-frame linear velocity tracking."""
    return track_lin_vel_xy_base_frame_exp(env, std, command_name, asset_cfg)


def track_ang_vel_z_base_frame_exp(
    env, command_name: str, std: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
    """Reward base-frame yaw-rate tracking using an exponential kernel."""
    # extract the used quantities (to enable type-hinting)
    asset = env.scene[asset_cfg.name]
...
def track_ang_vel_z_world_exp(
    ...
    """Backward-compatible name for base-frame yaw-rate tracking."""
    return track_ang_vel_z_base_frame_exp(env, command_name, std, asset_cfg)
